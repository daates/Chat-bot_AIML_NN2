...
namespace TopoBotCSharp
{
    ...
    public class Sample
    {
        ...
        /// </summary>
        public SignType actualClass;

        /// <summary>
        /// Распознанный класс - определяется после обработки
        /// </summary>
        public SignType recognizedClass;
        ...
        public Sample(double[] inputValues, int classesCount, SignType sampleClass = SignType.Undef)
        {
            //  Клонируем массивчик
            input = (double[]) inputValues.Clone();
            Output = new double[classesCount];
            if (sampleClass != SignType.Undef) Output[(int) sampleClass] = 1;


            recognizedClass = SignType.Undef;
        ...
        /// </summary>
        public SignType ProcessPrediction(double[] neuralOutput)
        {
            Output = neuralOutput;
            if (error == null)
            ...
            for (int i = 0; i < Output.Length; ++i)
            {
                ...
                if (Output[i] > Output[(int) recognizedClass]) recognizedClass = (SignType) i;
            }
